...
get_data_arbitrary <- function(det=NULL, 
  ...
  if(tolower(data_format) == "otn") {
    #Before we do any of the below we need to properly map the OTN data- if we have it- to IMOS data. Right now I need to split this into two distinct types of call.
    #This needs to be fixed with a more elegant call in Surimi, i.e, map_otn_file should be able to take receiver and tag dataframes for pass-thru to imos_otn_column_map, but for the purposes of
    #testing and getting this code off the ground, this will be fine for now.
    
    #If we don't have receiver and tag metadata we need to derive it using surimi.
    if(is.null(rmeta) && is.null(tmeta)) {
      ...
      extension <- tools::file_ext(det)
      if (extension == "parquet") {
        # If the file is a parquet file, then just call the appropriate mapping function.
        mapped_list <- surimi::otn_imos_new_style_column_map(det, rmeta, tmeta, derive = FALSE, tagname_column = "tagName")
      }
      else {
        col_check <- read.csv(det, nrows = 3)
        columns = names(det)
      ...
      det_data <- mapped_list$detections
      rec_meta <- mapped_list$receivers
      tag_meta <- mapped_list$tags
    }
  }
  ...
  if(!is.null(rmeta)) {
    #If we're dealing with an inputlist generated by createInputList then our aggregated receiver metadata will be coming in as a dataframe, not a path to a CSV file, and will have already been mapped above. But if it's
    #a character vector (i.e, a filepath) we still need to load it for IMOS' sake. 
    if(is.character(rmeta)) {
  ...
  if(!is.null(tmeta)) {
    #Same as receivers, if it's a dataframe coming in from createInputList then we need to load it as such. 
    if(is.character(tmeta)) {
  ...
  if(!is.null(tag_meta)) {
    ...
    dd <- dd %>% mutate(
      transmitter_deployment_latitude = as.numeric(transmitter_deployment_latitude)
    )
    dd <- dd %>% mutate(
      transmitter_deployment_longitude = as.numeric(transmitter_deployment_longitude)
    )
  }
